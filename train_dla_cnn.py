...
def evaluate(model: DlaCnnModel,
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            all_count_pred.append(outputs["count_logit"].argmax(dim=1).cpu().numpy())
            all_count_true.append(batch["n_dla"].cpu().numpy())
            all_params.append(outputs["params"].cpu().numpy())
    ...
    metrics["z_rmse"] = rmse_masked(params[:, [0, 2]], targets[:, [0, 2]], masks[:, [0, 2]])
    metrics["lognhi_rmse"] = rmse_masked(params[:, [1, 3]], targets[:, [1, 3]], masks[:, [1, 3]])
    return metrics
